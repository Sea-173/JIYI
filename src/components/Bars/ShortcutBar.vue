<template>
  <div class="shortcut-bar">
    <div class="container">
      <span class="welcome">欢迎来到JIYI！</span>
      ...
      <ul class="right-part">
        ...
        <li class="user-center" @mouseenter="userCenterDown" @mouseleave="userCenterDown">
          ...
          <ul v-show="isUserCenterDown">
            <li v-for="item in userCenter">{{ item.name }}<span v-show="item.number" class="item-number"> {{
                item.number
              }}</span></li>
          </ul>
        </li>
        ...
        <li class="business-cooperation" @mouseenter="businessCooperationDown" @mouseleave="businessCooperationDown">
          ...
          <ul v-show="isBusinessCooperationDown">
            <li v-for="item in businessCooperation" @click="gotoPage(item)">{{ item.name }}</li>
          </ul>
        </li>
        <li class="app-qrcode" @mouseenter="showAppQrcode" @mouseleave="showAppQrcode">
          ...
          <div class="qrcode" v-show="isShowAppQrcode">
            <p>扫码下载手机APP</p>
            <img src="../../assets/img/app-header/app-qrcode.png">
          </div>
        </li>
      </ul>
    </div>
...
.shortcut-bar .address .current-address {
  ...
  border-top: none;
  background: url('../../assets/img/app-header/icons/address.png') no-repeat;
...
.shortcut-bar .address:hover .current-address {
  color: #12ab34;
  background-image: url('../../assets/img/app-header/icons/address-active.png');
}
