  ...
  <div class="good-message" >
    <div lang="en-US" class="gitment-container gitment-editor-container">
      <div class="gitment-editor-main">
        <div class="gitment-editor-body">
          <div class="gitment-editor-write-field">
            <textarea placeholder="您的留言" title="Login to Comment" v-model="contentData" ></textarea>
          </div>
      ...
      <div class="gitment-editor-footer">
        <button class="gitment-editor-submit" title="Login to Comment" @click="funSubmit">留言</button>
      </div>
    </div>
    <div lang="en-US" class="gitment-container gitment-comments-container">
      <div class="overflowable-container" >
        <ul class="gitment-comments-list">
          <li class="gitment-comment" v-for="(comment,index) in commentsList" :key="index" >
            <a class="gitment-comment-avatar">
              <img class="gitment-comment-avatar-img" :src="getUserHead(comment.index)">
            </a>
            <div class="gitment-comment-main">
              <div class="gitment-comment-header">
                ...
                <div class="gitment-comment-like-btn">
                  <span  @click="addRating(index)" style="color: navajowhite" :id="getId(index)">❤</span>
                  {{comment.rating}}
                </div>
              </div>
  ...
  methods: {
    ...
    funSubmit: function () {
      for (let i = 0; i < this.commentsList.length; i++) {
        if (this.commentsList[i].user === this.messageRecord) {
          return
        }
      }
      var that = this
      ...
      if (this.contentData !== ' ') {
        that.commentsList.unshift({
          index: that.messageNum++,
          user: '张三',
          date: '' + date.getFullYear() + '.' + (date.getMonth() > 9 ? '' : '0') + (date.getMonth() + 1) + '.' + (date.getDate() > 9 ? '' : '0') + (date.getDate()),
          content: that.contentData,
          rating:0,
          isRating:false
        })
        this.messageRecord = '张三'
      }
      that.contentData = ' '
    },
    addRating: function (index){

      if(this.commentsList[index].isRating)
      {
        document.getElementById(this.getId(index)).style.color="navajowhite"
        if(this.commentsList[index].rating === 0)
        {
          return;
        }
        else {
          this.commentsList[index].rating--;
        }
        this.commentsList[index].isRating=false;
      }
      else
      {
        document.getElementById(this.getId(index)).style.color="red"
        this.commentsList[index].rating++;
        this.commentsList[index].isRating=true;
      }
    }
  },
}
