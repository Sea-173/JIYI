  		...
  		<div class="container">
	  		...
	  		<div class="shopping-cart" @mouseenter="showShoppingCartList" @mouseleave="showShoppingCartList">
		  		...
		  		<div class="shopping-cart-list" v-show="isShowShoppingCartList">
	  				...
	  				<ul>
	  					<li v-for="product in myShoppingCart" class="product-item" v-if="product.isshow">
	  						<img :src="productImageURL(product.id)">
	  						<div class="product-info">{{ product.name }}</div>
	  						<div class="product-number">
	  							<p><span class="price">￥{{ product.price }}</span> × {{ product.number }}</p>
	  							<button class="delete-product" @click="delete_item($event,product)">删除</button>
	  						</div>
	  					</li>
	  				</ul>
	  				<div class="total">
	  		...
	  		<div class="search-box">
	  			<form class="search">
		  			<label class=""><input type="text" name="" placeholder="搜索 " value="二手书 "><span>搜索</span></label>
		  		</form>
		  		<p class="hot-search">
...
export default {
	...
	computed: {
        ...
        if (this.myShoppingCart[i].isshow === 0)
        {continue}
      ...
      this.isShowShoppingCartList = !this.isShowShoppingCartList
		...
		showShoppingCartList: function () {
	  	...
	  	productImageURL: function (id) {
			// require用法
			return require('../../assets/img/product-img/' + id + '.jpg');
		},
...
<style scoped>
	...
	.search-area .logo {
		...
		width: 180px;
		top: 15px;
		background: url('../../assets/img/main-view/logo1.png') no-repeat;
